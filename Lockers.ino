...
const char *ssid = "EDIC_HUB_LAN1";
const char *password = "EDIC$1234";

AsyncWebServer server(80);

IPAddress local_IP(199, 168, 68, 184); // Note: Corrected the first octet to a valid private range value
...
void setup() {
  ...
  server.on("/details", HTTP_POST, 
    // Success Response Handler (placeholder)
    [](AsyncWebServerRequest *request){},
    // File Upload Handler (not used for body)
    NULL,
    // Body Handler (where all the magic happens)
    [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total){
      // When the first chunk of data arrives, create the temporary file
      if (index == 0) {
        ...
        file.close(); // Close immediately, will reopen in append mode
      }

      // Write each chunk of data to the temporary file
